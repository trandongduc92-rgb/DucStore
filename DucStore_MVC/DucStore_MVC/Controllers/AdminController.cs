...
namespace DucStore_MVC.Controllers
{
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            ViewBag.PendingOrders = db.DonHang.Count(d => d.TrangThai == "Chờ duyệt");
            ViewBag.TotalCustomers = db.KhachHang.Count;
            ViewBag.TotalRevenue = db.DonHang.Where(d => d.TrangThai == "Đã duyệt" || d.TrangThai == "Đã giao").Sum(d => d.TongTien);

            ViewBag.RecentOrders = db.DonHang.OrderByDescending(d => d.NgayDat).Take(5).ToList();
            ViewBag.Categories = db.DanhMuc;
            ViewBag.Products = db.SanPham;
            ViewBag.Orders = db.DonHang.OrderByDescending(d => d.NgayDat).ToList();
